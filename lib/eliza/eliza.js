...
Eliza.prototype._preFilter = function(script) {
  return function(phrase) {
    for (var pre_key in script.pre) {
      if(script.pre.hasOwnProperty(pre_key)) {
        replacement = script.pre[pre_key];
        phrase = phrase.replace(new RegExp(pre_key, "i"), replacement);
      }
    }
    return phrase;
  }
...
Eliza.prototype._postFilter = function(script) {
  return function(phrase) {
    for (var post_key in script.post) {
      if(script.post.hasOwnProperty(post_key)) {
        replacement = script.post[post_key];
        phrase = phrase.replace(new RegExp(post_key, "i"), replacement);
      }
    }
    return phrase;
  }
...
Eliza.prototype._createDecomp = function(match_string, phrases, post) {
  ...
  var regex_string = match_string.replace(/\s*\*\s*/g, "\\s*(.*?)\\s*");
